...
with open(log_path, "r", encoding="utf-8") as f:
    for line in f:
        ...
        if data.get("step_index") in [88, 125, 950]:
            res.append(f"--- STEP {data.get('step_index')} ---")
            ...
            for call in tool_calls:
                ...
                for k, v in args.items():
                    if isinstance(v, str) and ("GVCN" in v or "Biên bản" in v):
                        ...
                        res.append(v[max(0, idx-150):min(len(v), idx+150)])
                        
with open("check_steps_output.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(res))
