...
switch_new = """    const switchTab = (tab) => {
        ...
        const btns = document.querySelectorAll('.zalo-tab-btn');
        if (btns.length >= 5) {
            btns[0].className = `zalo-tab-btn flex-1 py-3 px-4 rounded-xl font-semibold flex items-center justify-center gap-2 ${activeTab === 'contacts' ? 'bg-white shadow-md text-blue-600' : 'text-slate-600 hover:bg-slate-100'} transition-all min-width:140px;`;
            btns[1].className = `zalo-tab-btn flex-1 py-3 px-4 rounded-xl font-semibold flex items-center justify-center gap-2 ${activeTab === 'media' ? 'bg-white shadow-md text-purple-600' : 'text-slate-600 hover:bg-slate-100'} transition-all min-width:140px;`;
            btns[2].className = `zalo-tab-btn flex-1 py-3 px-4 rounded-xl font-semibold flex items-center justify-center gap-2 ${activeTab === 'errors' ? 'bg-white shadow-md text-blue-600' : 'text-slate-600 hover:bg-slate-100'} transition-all min-width:140px;`;
            btns[3].className = `zalo-tab-btn flex-1 py-3 px-4 rounded-xl font-semibold flex items-center justify-center gap-2 ${activeTab === 'proctor' ? 'bg-white shadow-md text-blue-600' : 'text-slate-600 hover:bg-slate-100'} transition-all min-width:140px;`;
            btns[4].className = `zalo-tab-btn flex-1 py-3 px-4 rounded-xl font-semibold flex items-center justify-center gap-2 ${activeTab === 'bulk' ? 'bg-white shadow-md text-blue-600' : 'text-slate-600 hover:bg-slate-100'} transition-all min-width:140px;`;
        }
        renderTabContent();
        if (typeof lucide !== 'undefined') lucide.createIcons();
    };"""
...
# 4. Insert the huge Media Engine before "return {"
media_engine = """
    // ==========================================
...
# Append the media_engine before the final return statement
export_old_updated = """    return {
        init,
        switchTab,
        updateClassPhone,
        updateGroupLink,"""

# Since we don't know the exact export yet, let's just find `return {` at the end
# and insert our exported functions.
import re
match = re.search(r'    return \{(.*?)\};$', text, re.DOTALL | re.MULTILINE)

if match:
    # Insert media_engine right before the return statement
    insertion_idx = match.start()
    text = text[:insertion_idx] + media_engine + text[insertion_idx:]
    
    # Update exports
    export_block = match.group(1)
    new_exports = export_block + """,
        mediaSelectClass,
        mediaSelectStudent,
        mediaSelectTemplate,
        startMediaQueue,
        ...
        copyCurrentMedia
"""
    text = text[:insertion_idx + len(media_engine)] + "    return {" + new_exports + "};\n})();\n"
else:
    print("Could not find the end return block")
