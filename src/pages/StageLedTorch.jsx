...
export default function StageLedTorch() {
  ...
  // 2. CANVAS RENDERING: FLYING FIREBALL ARC & SOARING SKY FIREWORKS GALAXY
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (activeStep >= 10) { // SOARING & BURST
        bgGrad.addColorStop(0, 'rgba(180, 83, 9, 0.5)');
        bgGrad.addColorStop(0.5, 'rgba(153, 27, 27, 0.4)');
      ...
      // ----------------------------------------------------
      // 2. STEP 10: SOARING FIREBALL TO HIGH SKY (Person 5 -> High Sky Center)
      // ----------------------------------------------------
      if (activeStep === 10) {
        if (flyProgressRef.current < 1.0) {
          flyProgressRef.current += 0.022; // ~1.5s vertical flight
          if (flyProgressRef.current >= 1.0) {
            flyProgressRef.current = 1.0;
            // Spawn Firework Galaxy Burst & move to Step 11 (Grand Anniversary Banner)
            const targetSkyX = width / 2;
            const targetSkyY = height * 0.22;
            for (let i = 0; i < 150; i++) {
              fireworkSparks.push(new FireworkSpark(targetSkyX, targetSkyY));
            }
            setActiveStep(11);
            playAudioFx();
          }
        ...
        const pSky = { x: width / 2, y: height * 0.22 };
        const pSkyCtrl = { x: (pStart.x + pSky.x) / 2 + 50, y: Math.min(pStart.y, pSky.y) - 60 };
  ...
  return (
    <div style={styles.stageContainer}>
      ...
      <div style={styles.stageOverlay}>
        ...
        {/* GRAND FINALE ANNIVERSARY BANNER (ACTIVE ON STEP 11) */}
        {activeStep >= 11 ? (
          <div style={{ textAlign: 'center', animation: 'bannerScaleUp 1s cubic-bezier(0.16, 1, 0.3, 1) forwards', marginTop: '10px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 20px', borderRadius: '30px', backgroundColor: 'rgba(185, 28, 28, 0.85)', border: '2px solid #fef08a', boxShadow: '0 0 30px rgba(245, 158, 11, 0.6)', marginBottom: '10px' }}>
              <Trophy size={20} color="#fef08a" />
              <span style={{ fontSize: '14px', fontWeight: '900', color: '#ffffff', letterSpacing: '2px', textTransform: 'uppercase' }}>
                TRƯỜNG THPT CAO BÁ QUÁT • NĂM 1996 ✦ 2026
              </span>
            </div>

            <h1 style={styles.grandAnniversaryHeading}>
              ĐẠI LỄ KỶ NIỆM 30 NĂM THÀNH LẬP
            </h1>
            
            <div style={{ fontSize: '24px', fontWeight: '900', color: '#fef08a', letterSpacing: '2px', marginTop: '4px', textShadow: '0 4px 15px rgba(0,0,0,0.9)' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: '10px' }}>
            ...
            <h1 style={styles.mainTitle}>
              {customTitle || (
                activeStep === 10 ? '🚀 CẦU LỬA THIÊNG VÚT BAY LÊN TRỜI CAO...' :
